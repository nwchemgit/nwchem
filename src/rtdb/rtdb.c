...
*/
{
  ...
  /* See if the data base is already open ... if so just return */
  ...
  if (strcmp(mode, "new") == 0) {
    if (exists) {
      (void) fprintf(stderr, "rtdb_open: %s exists but is being opened new\n",
		     filename);
      return 0;
    }
  }
    ...
    if (!exists) {
      (void) fprintf(stderr, 
		     "rtdb_open: %s does not exist but is being opened old\n",
		     filename);
      return 0;
    }
  }
    ...
    (void) fprintf(stderr, "rtdb_open: unknown mode=%s\n", mode);
    return 0;
  }
  ...
  for (new=0; new<MAX_RTDB && rtdb[new].active; new++)
    ;
  if (new == MAX_RTDB) {
    (void) fprintf(stderr, "rtdb_open: too many data bases open, max=%d\n",
		   MAX_RTDB);
    return 0;
  }
  ...
  if (!(rtdb[new].db = 
    ...
    (void) fprintf(stderr, "rtdb_open: db failed to open file %s\n",
		   filename);
    return 0;
  }
...
*/
{
  ...
  if (status == 0) {
    if (namelen >= key.size) {
      ...
      (void) fprintf(stderr, "rtdb_first: name is too small, need=%u got=%d\n",
...
*/
{
  ...
  if (status == 0)
    ...
    (void) fprintf(stderr, "rtdb_delete: error deleting %s\n", name);
    return 0;
  }
}
...
*/
{
  ...
  if (status == 0) {
    if (namelen >= key.size) {
      ...
      (void) fprintf(stderr, "rtdb_next: name too small, need=%u, got=%d\n",
		     (int) key.size, (int) namelen);
      return 0;
    }
  }
...
*/
{
  ...
  if (!check_handle(handle)) {
    (void) fprintf(stderr, "rtdb_print: handle (%d) is invalid\n", handle);
    return 0;
  }
  ...
  status = rtdb_first(handle, sizeof(name), name);
  ...
  while (status) {
    ...
    if (!rtdb_get_info(handle, name, &ma_type, &nelem, date)) {
      (void) fprintf(stderr, "rtdb_print: get_info failed on %s\n", name);
    }
	...
	if (!rtdb_ma_get(handle, name, &ma_type, &nelem, &ma_handle)) {
	  (void) fprintf(stderr, "rtdb_print: ma_get failed on %s\n", name);
	}
	else {
	  void *data;
	  if (MA_get_pointer(ma_handle, &data))
	    ...
	    (void) fprintf(stderr, "rtdb_print: MA_get_pt failed, handle=%d\n",
			   handle);
	  (void) MA_free_heap(ma_handle);
	}
      }
    }
    status = rtdb_next(handle, sizeof(name), name);
  }
  printf("\n");
...
static int rtdb_put_info(const int handle,
...
*/  
{
  ...
  if (!check_handle(handle)) {
    (void) fprintf(stderr, "rtdb_put_info: handle (%d) is invalid\n", handle);
    return 0;
  }
  ...
  if (strlen(name)+sizeof(info_header)+1 > sizeof(info_buf)) {
    (void) fprintf(stderr,"rtdb_put_info: info entry buffer needs to be %d\n",
  ...
  if (db->put(db, &key, &value, (u_int) 0)) {
    (void) fprintf(stderr, "rtdb_put_info: db put failed for \"%s\" in %s\n",
...
*/  
{
  ...
  if (!check_handle(handle)) {
    (void) fprintf(stderr, "rtdb_get_info: handle (%d) is invalid\n", handle);
    return 0;
  }

  if (strlen(name)+sizeof(info_header)+1 > sizeof(info_buf)) {
    (void) fprintf(stderr,"rtdb_get_info: info entry buffer needs to be %d\n",
		   (int) (strlen(name)+sizeof(info_header)+1));
    return 0;
  }
  ...
  if (db->get(db, &key, &value, (u_int) 0)) {
    ...
    if (status == -1) {
      (void) fprintf(stderr, "rtdb_get_info: db get failed for \"%s\" in %s\n",
		     name, rtdb[handle].filename);
    }
    else {
  ...
  if (value.size != sizeof(info)) {
    (void) fprintf(stderr, "rtdb_get_info: size mismatch : info=%d, db=%d\n",
...
*/  
{
  ...
  if (!check_handle(handle)) {
    (void) fprintf(stderr, "rtdb_put: handle (%d) is invalid\n", handle);
    return 0;
  }
  ...
  if (db->put(db, &key, &value, (u_int) 0)) {
    (void) fprintf(stderr, "rtdb_put: db put failed for \"%s\" in %s\n",
		   name, rtdb[handle].filename);
    return 0;
  }
  ...
  status = rtdb_put_info(handle, name, ma_type, nelem);
...
*/
{
  ...
  if (!check_handle(handle)) {
    (void) fprintf(stderr, "rtdb_get: handle (%d) is invalid\n", handle);
    return 0;
  }
  ...
  if (!rtdb_get_info(handle, name, &ma_type_from_db, &nelem_from_db, date)) {

    /* In production will want to have this handled quietly  ... be
       verbose for debug only 

    (void) fprintf(stderr, "rtdb_get: get info failed for \"%s\" in %s\n",
		   name, rtdb[handle].filename); */
    return 0;
  }
  ...
  if (ma_type_from_db != ma_type) {
    (void) fprintf(stderr, 
		   "rtdb_get: type mismatch for \"%s\" in %s: arg=%d, db=%d\n",
  ...
  if ((status = db->get(db, &key, &value, (u_int) 0))) {
    ...
    (void) fprintf(stderr, "rtdb_get: db get failed for \"%s\" in %s\n",
		   name, rtdb[handle].filename);
    return 0;
  }
  ...
  if (nelem_actual != nelem_from_db) {
    (void) fprintf(stderr, 
		   "rtdb_get: size error for \"%s\" in %s: info=%d, db=%d\n",
  ...
  if (nelem_actual > nelem) {
    (void) fprintf(stderr, "rtdb_get: \"%s\" is %d long, buffer is only %d\n",
...
*/
{
  ...
  if (!check_handle(handle)) {
    (void) fprintf(stderr, "rtdb_get_ma: handle (%d) is invalid\n", handle);
    return 0;
  }
  ...
  if (!rtdb_get_info(handle, name, ma_type, nelem, date)) {
    (void) fprintf(stderr, "rtdb_get_ma: get info failed for \"%s\" in %s\n",
		   name, rtdb[handle].filename);
    return 0;
  }
  ...
  if ((status = db->get(db, &key, &value, (u_int) 0))) {
    if (status == -1) {
      (void) fprintf(stderr, "rtdb_get_ma: db get failed for \"%s\" in %s\n",
		     name, rtdb[handle].filename);
    }
    else {
      /* Entry not found ... quietly return error so that failed
  ...
  if (nelem_actual != *nelem) {
    (void) fprintf(stderr, 
		   "rtdb_get_ma: size error for \"%s\" in %s: %d != %d\n",
  ...
  if (!MA_allocate_heap(*ma_type, *nelem, name, ma_handle)) {
    (void) fprintf(stderr, "rtdb_get_ma: MA_allocate_heap failed, nelem=%d\n",
		   *nelem);
    return 0;
  }
  
  if (!MA_get_pointer(*ma_handle, &ma_data)) {
    (void) fprintf(stderr, "rtdb_get_ma: MA_get_pointer failed\n");
    return 0;
  }
