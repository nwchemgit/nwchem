/* Index permutation routines with accumulation in C for IA-32 processors
 * (interface compatible with IP_acc routines in nwchem/tce)
...
void copyacc(double* a,double* b,long size,double factor) {
  int i;
  for (i = 0; i < size; i++)
    b[i] += a[i]*factor;
}

void tce_sortacc_0_(double* unsorted,double* sorted,double* factor) {
  *sorted += (*unsorted) * (*factor);
}
    
    
void transposeacc_aligned(double *a, double *b, int N1, int N2, double factor) {

  int i,j,k,k1,it,jt,itt,jtt,conflict,tmp,tmpN;
  double *pA, *pB;

  register __m128d x, y, z, w,p,q,r,s,fac_vector; 
  ...
  for (it = 0; it < N1; it=it+tilesize) {      
	...
	p = _mm_load_pd(pB);
	q = _mm_load_pd(pB + 2);
	r = _mm_load_pd(pB + 2*2);
	s = _mm_load_pd(pB + 3*2);
	x = _mm_add_pd(x,p);
	y = _mm_add_pd(y,q);
	z = _mm_add_pd(z,r);
	w = _mm_add_pd(w,s);
...
void transposeacc_misaligned(double *a, double *b, int N1, int N2, double factor) {
  ...
  for (itt = 0; itt < itt_bound; itt=itt+5*tilesize) {
	...
	b[j*N1+i] = factor*a[i*N2+j] +  b[j*N1+i];
      }
    }
  }
...
void tce_sortacc_2_(double* unsorted,double* sorted,
  ...
  //check if trivial
  if (i==1 && j==2) {
    copyacc(unsorted,sorted,a*b,factor);
    return;
  }
  ...
  if (a <= 0 || b <= 0) {
    ...
    transposeacc_aligned(unsorted,sorted,a,b,factor);
  } else {
    // transpose_misaligned
    transposeacc_misaligned(unsorted,sorted,a,b,factor);
  }    
}

void tce_sortacc_4_scalar(double* unsorted,double* sorted,
...
void tce_sortacc_4_simd(double* unsorted,double* sorted,
			int a, int b, int c, int d, 
			int i, int j, int k, int l,
			double factor) {
  int id[4],jd[4],ia,ib,j1,j2,j3,j4;
...
void tce_sortacc_4_(double* unsorted,double* sorted,
  ...
  if (dim[l]%2 == 0 && dim[3]%2 == 0)
    tce_sortacc_4_simd(unsorted,sorted,a,b,c,d,i,j,k,l,factor);
  else
    tce_sortacc_4_scalar(unsorted,sorted,a,b,c,d,i,j,k,l,factor);
}

void tce_sortacc_6_simd(double* unsorted,double* sorted,
...
void tce_sortacc_6_scalar(double* unsorted,double* sorted,
			  int a, int b, int c, int d, int e, int f,  
			  int i, int j, int k, int l, int m, int n,
			  double factor) {
  int id[6],jd[6],ia,ib,j1,j2,j3,j4,j5,j6;
...
void tce_sortacc_6_(double* unsorted,double* sorted,
  ...
  if (dim[n]%2 == 0 && dim[5]%2 == 0)
    tce_sortacc_6_simd(unsorted,sorted,a,b,c,d,e,f,i,j,k,l,m,n,factor);
  else
