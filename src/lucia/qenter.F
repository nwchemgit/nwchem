...
C     TIMEX = mCLOCK(*)
*. Cray
C     TIMEX = 100.0D0*SECOND()
*.Linux environment
C     TIMEX = 100.0D0*TIMELINUX()
*. Regatta
      CALL CPU_TIME(USTIME)
      TIMEX = 100.0D0*USTIME
      RETURN
      END
c
      FUNCTION WIMEX()
