...
*
      ...
      RETURN
      END
      SUBROUTINE SBLOCKSN(NSBLOCK,ISBLOCK,CB,SB,C2,
     &           ICOCOC,ICSMOS,ICBLTP,NACOB,NSSOA,NSSOB,
     &           NAEL,IAGRP,NBEL,IBGRP,
     &           IOCTPA,IOCTPB, NOCTPA,NOCTPB,
     &           NSMST,NSMOB,NSMSX,NSMDX,NOBPTS,IOBPTS,MXPNGAS,
     ...
     &           LUCBLK,IPERTOP,IH0INSPC,IH0SPC,
     &           ICBAT_RES,ICBAT_INI,ICBAT_END,IUSE_PH,IPHGAS,
     &           I_RES_AB,IUSE_PA,CJPA,SIPA,ISIMSYM,
     &           IREO_IA,IREO_IB,IREO_JA,IREO_JB,IUSE_HW)
*
* Direct RAS routine employing combined MOC/n-1 resolution method
*
* Jeppe Olsen , Winter of 1991
*               Last modification : June 99, ISIMSYM business added
*                                 
*
* =====
* Input
...
*. Will this block be needed ??
          INTERACT = 0
C         IF(SCLFAC(JBLOCK).EQ. 1.0D0) THEN
...
*. Loop over C blocks
        ...
        DO 9000 ICBLK = I1CBLOCK(JCBATCH), I1CBLOCK(JCBATCH)-1+NJBLOCK 
          ...
          NJB = NSSOB(JBSM,JBTP)
          IF(NTEST.GE.100) THEN
            WRITE(6,*) ' Next block of C, ICBLK,TTSS ',
     &      ICBLK,JATP,JBTP,JASM,JBSM 
          END IF
*
C         IF(SCLFAC(ICBLK).NE.0.0D0) THEN
*         ^ Commented out in order to have the ISIMSYM option working.
*           Should however, be reinstated somehow ....
*. Other symmetry blocks that can be obtained from this block
            PL = 1.0D0
            CALL PRMBLK(IDC,ISTRFL,JASM,JBSM,JATP,JBTP,PS,PL,
     &                  LATP,LBTP,LASM,LBSM,LSGN,LTRP,NPERM)
...
*
                IF(IPERM.EQ.1) THEN 
*. Transpose all blocks in BATCH
                 KSTRT = I1CBLOCK(JCBATCH)
                 KSTOP = I1CBLOCK(JCBATCH)-1+NJBLOCK 
                 DO KCBLK = KSTRT, KSTOP                                   
                   KATP = ICBLOCK(1,KCBLK)
                   KBTP = ICBLOCK(2,KCBLK)
                   KASM = ICBLOCK(3,KCBLK)
                   KBSM = ICBLOCK(4,KCBLK)
                   KCOFF = ICBLOCK(5,KCBLK)
                   NKA = NSSOA(KASM,KATP)
                   NKB = NSSOB(KBSM,KBTP)
                   IF(IDC.EQ.2.AND.KATP.EQ.KBTP.AND.KASM.EQ.KBSM) THEN
*. Diagonal blocks, Transposing corresponds to scaling
                     IF(PS.EQ.-1.0D0) THEN
                      CALL SCALVE(CB(KCOFF),PS,NKA*NKB)
                     END IF
                   ELSE
*. ofdiagonal blocks, explicit transposing
                    CALL TRPMT3(CB(KCOFF),NKA,NKB,C2)   
                    CALL COPVEC(C2,CB(KCOFF),NKA*NKB)   
                   END IF
                 END DO
                END IF
*               ^ End if transposing
              ELSE 
                IF(LLATP_P.NE.LLATP.OR.LLBTP_P.NE.LLBTP) THEN
                  INEW_CTT = 1
                ELSE
                  INEW_CTT = 0
                END IF
              END IF
C?            WRITE(6,*) ' ICBLK, INEW_CTT, LLASM, LLBSM', 
C??  &                     ICBLK, INEW_CTT, LLASM, LLBSM
C?            WRITE(6,*) ' LLATP, LLBTP, LLATP_P, LLBTP_P', 
C?   &                     LLATP, LLBTP, LLATP_P, LLBTP_P 
C?            WRITE(6,*) ' IC1BLK ', IC1BLK
*. The routines assumes on input that the blocks are transposed, so,
*. Initial tour, IPERM = 1 corresponds always to no transpose, so transpose!
CM            IF(IPERM.EQ.1) THEN       
CM              IF(IDC.EQ.2.AND.JATP.EQ.JBTP.AND.JASM.EQ.JBSM) THEN
*. Diagonal blocks, Transposing corresponds to scaling
CM                IF(PS.EQ.-1.0D0) THEN
CM                  CALL SCALVE(CB(ICOFF),PS,NJA*NJB)
CM                END IF
CM              ELSE
*. ofdiagonal blocks, explicit transposing
CM                CALL TRPMT3(CB(ICOFF),NJA,NJB,C2)   
CM                CALL COPVEC(C2,CB(ICOFF),NJA*NJB)   
CM              END IF
CM            END IF
*
              IATP_P = -0
              IBTP_P = -0
              DO 10000 ISBLK = 1, NSBLOCK
CM            IF(ISBLOCK(1,ISBLK) .GT. 0 ) THEN
                IF(ISBLK.GT.1) THEN
                  IATP_P = IATP
                  IBTP_P = IBTP
                END IF
                IATP = ABS(ISBLOCK(1,ISBLK))
                IBTP = ISBLOCK(2,ISBLK)
*
                IF(ISBLK.EQ.1) THEN
                  INEW_STT = 1
                ELSE 
                  IF(IATP_P.NE.IATP.OR.IBTP_P.NE.IBTP) THEN
                    INEW_STT = 1
                  ELSE
                    INEW_STT = 0
                  END IF
...
*
     ...
     &            GOTO 8764
*
C?              IF(ISBLK.EQ.61.AND.(ICBLK.EQ.1.OR.ICBLK.EQ.5)) THEN
C?                NTEST = 1000
C?                IPRNT = 1000
C?              ELSE
C?                NTEST = 0
...
*
 ...
 8764           CONTINUE
                END IF
*               ^ End if S-block should be calculated
            IF(ISIMSYM.EQ.1.AND.INEW_STT.EQ.1.AND.INEW_CTT.EQ.1 )THEN
                CALL RSBB2B_M(IATP,IBTP,NSSOA(1,IATP),NSSOB(1,IBTP),
     &               LLATP,LLBTP,NSSOA(1,LLATP),NSSOB(1,LLBTP),
     &               IAGRP,IBGRP,ISBLOCK(1,ISBLK),ICBLOCK(1,ICBLK),
     &               NGAS,
     &               NELFSPGP(1,IATP+IOCTPA-1),
     &               NELFSPGP(1,IBTP+IOCTPB-1),
     &               NELFSPGP(1,LLATP+IOCTPA-1),
     &               NELFSPGP(1,LLBTP+IOCTPB-1),
     &               SB(ISOFF),CB(ICOFF),NOBPTS,MAXK,
...
* Order 
      DO  ISBLK = 1 , NSBLOCK
C     IF(ISBLOCK(1,ISBLK).GT.0) THEN
        IATP = ABS(ISBLOCK(1,ISBLK))
        IBTP = ISBLOCK(2,ISBLK)
        IASM = ISBLOCK(3,ISBLK)
        IBSM = ISBLOCK(4,ISBLK)
        ...
        NIA = NSSOA(IASM,IATP)
        NIB = NSSOB(IBSM,IBTP)
C?      WRITE(6,*) ' ISBLK, ISOFF, SB(ISOFF)',ISBLK,ISOFF,SB(ISOFF)
        IF(ICJKAIB.NE.0) THEN
*. Tranpose sigma block was obtained, transpose to obtain correct block
           CALL TRPMT3(SB(ISOFF),NSSOB(IBSM,IBTP),
     &                 NSSOA(IASM,IATP),C2)
