% $Id: interface.tex,v 1.12 2002-02-13 21:14:37 windus Exp $
\label{sec:interface}
...
There are two $H + H_2$ tests.
Both use the Euler integration method at the SCF/3-21G level
of theory to calculate points along the reaction path.
The first one,  \verb+$NWCHEM_TOP/QA/tests/h3tr1+, calculates a few
points along the surface.  The second one, \verb+$NWCHEM_TOP/QA/tests/h3tr2+,
is a restart of the h3tr1 calculation.
...
The $OH + C_2H_4$ test uses the Page-McIver LQA method to calculate points
on the SCF/3-21G surface.  This test case is located in the
\verb+$NWCHEM_TOP/QA/tests/c2h5otr1+ directory.

Note: All of these tests are set up with SCF, however, other levels of
