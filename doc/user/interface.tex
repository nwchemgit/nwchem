% $Id: interface.tex,v 1.4 2001-02-06 22:22:59 windus Exp $
\label{sec:interface}
...
Output.  There one default output file for each \verb+DIRDYVTST+ run - .file30.
...
\end{verbatim}
  ...
  Descriptions
    ...
    TITLE is a keyword that allows user to input a description of
...
\end{verbatim}
    ...
    SINGLEPOINT is a keyword that specifies that a single
	      point calculation is to be performed at the optimized 
	      geometries.  The type of single point calculation is 
	      specified in the \verb+sptheory+ line.
...
\end{verbatim}
  ...
  is the input for the transition state if one exists.
...
\end{verbatim}
  ...
  Descriptions
    ...
    SSAVE is a variable keyword that indicates the numerical step
              ...
              derivatives are recalculated and written to the fort.30
              file. For example, if SSTEP=0.01 and SSAVE=0.1, then the
              potential information is written to fort.30 every 10
...
at the UHF$\backslash$3-21G level of theory and the singlepoint calculations are 
calculated at the MP2$\backslash$cc-pVDZ level of theory with a tighter convergence
