...
\hline
%           &          &             &        &         &         \\
Keyword    & Exchange & Correlation & GCA    &  Hybrid & Ref.\\
%           &          &             &        &         &         \\
\hline
 slater     & $\star$    &            &         &         &[1]\\
\hline							
...
\begin{verbatim}
  GRID [(xcoarse||coarse||medium||fine||xfine) default medium] \
       [(gausleg <integer radpts default 50> 
                 <integer nagrid default 10>) ||\ 
        (lebedev <integer radpts> 
                 <integer iangquad>)] \ 
       [lebmed||lebfine||lebxfine)
       [becke||erf1||erf2||ssf] \
       [euler||mura||treutler||lindh] \
       [rm <real rm default 2.0>]
\end{verbatim}
...
the level of angular quadrature (1-11).
...
\end{verbatim}
Work is in progress to define pre-computed Lebedev quadrature of give accuaracy
\begin{center}
  \begin{tabular}[right]{|c|c|} \hline
Keyword & {\tt Total Energy Target Accuracy} \\ \hline
{\tt lebmed}  & $1x10^{-6}$ \\ \hline
{\tt lebfine}    & $1x10^{-7}$ \\ \hline
{\tt lebxfine}   & $1x10^{-8}$ \\ \hline
  \end{tabular} \\
\end{center}
...
\begin{verbatim}
  GRID [(becke||erf1||erf2||ssf) default becke]
\end{verbatim}

{\tt ssf} R.E.Stratmann, G.Scuseria and  M.J.Frisch,
Chem. Phys. Lett. {\bf 257}, 213 (1996).


\begin{verbatim}
  GRID [[euler||mura||treutler||lindh]  default euler]
\end{verbatim}

{\tt euler} Murray Handy Laming, Euler-McLaurin \\
{\tt mura} M.E.Mura and P.J.Knowles, J Chem Phys {\bf 104}, 9848
(1996).\\
{\tt treutler} O.Treutler and R.Alrhichs, J.Chem.Phys {\bf 102}, 346 (1995).\\
{\tt lindh} R.Lindh (1999)
