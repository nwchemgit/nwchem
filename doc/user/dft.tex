...
\subsection{Input/Output of KS-MO Vector}
\begin{verbatim}
  VECTORS [[input] (<string input_movecs default atomic>) || \
                   (project <string basisname> <string filename>)] \
           [swap [alpha|beta] <integer vec1 vec2> ...] \
...
\subsubsection{Hybrid Exchange and Correlation Functionals}
...
\subsection{SCF Convergence Control}
\begin{verbatim}
...
\subsection{Controlling the Numerical Integration of the Exchange-Correlation Potential}
...
\subsection{Control of Screening (Setting Tolerances)}
\begin{verbatim}
  TOLERANCES [[tight] [tol_rho <real tol_rho default 1e-15>] \
              [accAOfunc <integer accAOfunc default 20>] \
              [accCoul <integer accCoul default 15>] \
...
\subsection{Hardware Resource Control}
\begin{verbatim}
  DIRECT|INCORE
  NOIO
\end{verbatim}
...
\subsection{Control of Open Shells}
\begin{verbatim}
  DFT|ODFT
  MULT <integer mult default 1>
\end{verbatim}
...
\subsection{Standard Property Control}
\begin{verbatim}
  MULLIKEN
\end{verbatim}
