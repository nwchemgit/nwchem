...
\section{{\tt SWITCH2E} --- overlap threshold which determines the method for calculating the two-electron contribution to the ET Hamiltonian}
\label{sec:switch2e}
...
In cases when the ET reactants and products are {\em very} weakly interacting (small $S_{RP}$), the two-electron
contribution to the electronic coupling should not be calculated with the NWChem Fock matrix routine (see \ref{sec:thresh}).
When $S_{RP}$ is less than \verb+switch2e+ , the \verb+NOFOCK+ routine will be used for
the two electron contribution to the coupling (see \ref{sec:etransfock}).  
