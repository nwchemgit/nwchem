...
may not be desirable for UHF wavefunctions, in which case specify the
\verb+NOADAPT+ keword (section \ref{sec:noadapt}).
...
If present, this keyword disables use of symmetry to speedup Fock
matrix construction (via the petite or skeleton algorithm) in the SCF
even if symmetry was used in the specification of the geometry.
Symmetry adaption of the molecular orbitals is still performed.
...
This keyword disables symmetry adaption of the molecular orbitals.
This does not impact the speed of the calculation but the resulting
orbitals may be symmetry contaminated, especially if starting a
calculation from orbitals from a distorted geometry.
...
This keyword generates and dumps the orbital Lagrangian to
disk. Lagrangian is required by ROHF gradients.
...
directive is useful, for example, to force the MCSCF to preserve the
ROHF ordering despite flipping of orbital energies.
...
overhead on machines which have expensive timing routines
(e.g., SUN).
...
open-shells. This is provided as a last resort option for cases where
the second-order scheme fails. Use the \verb+MAXITER+ directive to
increase the macroiteration count to approximately 20.
...
This controls the size of the subspace for DIIS extrapolation.
The default should be adequate in most instances but it may be reduced
to conserve memory for large systems or increased for problematic
convergence.
