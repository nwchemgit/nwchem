...
\item
The maximum number of solvent molecules on a single node can be set with
\begin{verbatim}
 mwm <integer mwmreq>
\end{verbatim}
where \verb+mwmreq+ is the maximum number of solvent molecules per
node, unless the value given is less than the computed number.
\item
The maximum number of solute atoms on a single node can be set with
\begin{verbatim}
 msa <integer msareq>
\end{verbatim}
