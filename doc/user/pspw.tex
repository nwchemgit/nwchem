...
...
   STEEPEST_DESCENT
      ...
      [EXCHANGE_CORRELATION: (Vosko || PBE96  default PBE96)]
      [MULLIKEN]
...
\end{eqnarray}
along with the ion kinetic energy
\begin{eqnarray}
\label{appendix:b2}
KE(\vec{R_I}) &=& \sum_{I} \frac{1}{2} M_I \left| \dot{\vec{R_I}} \right|^2  
\end{eqnarray}
...
\begin{eqnarray}
ELECTRON_THERMOSTAT(x_e) &=&  \frac{1}{2} Q_e \dot{x_e} + E_{e0} 
\end{eqnarray}
\begin{eqnarray}
ION_THERMOSTAT(x_R)      &=&  \frac{1}{2} Q_R \dot{x_R} + E_{R0} 
\end{eqnarray}
A description of the parameters $Q_e$, $E_{e0}$, $Q_R$, and $E_{R0}$ is given in 
section \ref{section:pspw_nose}.
...
...
   Car-Parrinello
      ...
      [EWALD_RCUT: <real rcut>]
      [EXCHANGE_CORRELATION: (Vosko || PBE96  default PBE96)]
      [Nose-Hoover: <real Period_electron Temperature_electrion Period_ion Temperatur_ion 
...
\begin{itemize}
	...
	\item $<$rcut$>$ - value for the cutoff radius used
        ...
        \item Nose-Hoover: - optional subblock which if specified
                         ...
                         \begin{itemize}
                             \item $<$Period\_electron$>$ - estimated period for electron motion
                             \item $<$Temperature\_electron$>$ - temperature for electron motion
                             \item $<$Period\_ion$>$ - estimated period for ion motion
                             \item $<$Temperature\_ion$>$ - temperature for ion motion
                         \end{itemize}
...
\section{PSPW Nose-Hoover Simulation Parameters}
\label{section:pspw_nose}
