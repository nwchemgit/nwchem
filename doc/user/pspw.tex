...
and section \ref{sec:pspw_nose} describes the input parameters for a Car-Parrinello
simulation at constant temperature.  Then the RTDB entries and datafiles used by the 
PSPW module are described in section \ref{sec:pspw_data}, and Car-Parrinello output data files are 
described in section \ref{sec:pspw_cp_data}.  Examples of how to setup and run
...
is described in section \ref{sec:pspw_sd}
        ...
        \item Nose-Hoover: - optional subblock which if specified
                         ...
                         \begin{itemize}
                             \item $<$Period\_electron$>$ $\equiv$ $P_{electron}$ 
                                    - estimated period for electron motion
                             \item $<$Temperature\_electron$>$ $\equiv$ $T_{electron}$ 
                                    - temperature for electron motion
                             \item $<$Period\_ion$>$ $\equiv$ $P_{ion}$ 
                                    - estimated period for ion motion
...
To perform Mulliken analysis information is needed from the one-dimensional
pseudopotential file.  In-order to facilitate the transfer of this information
...
\section{Minimizing the one-electron orbitals: Running a Steepest Descent and Conjuagate Gradient Simulation in Tandem}
\label{sec:pspw_sd}
...
methods that are used to minimize the DFT energy functional 
(Eqs.~\ref{eqn:lda1}-\ref{eqn:lda2}) are presented.
...
Eq.~\ref{eq:sd4} in the following form\cite{Car-Parrinello:88}.
\begin{equation}
...
search direction or line.  However, the equation for a line (or search direction)
in the LSDA minimization problem is complicated by the fact that the
energy functional is subject to orthonormality constraints of Eq.~\ref{eq:lda4}. 
...
These two pesky orthonormality constraint problems
...
\footnote{See reference on Lie Groups, e.g. reference\cite{Olver:95}, 
          for a definition of a quotient space and a definition of the 
...
conjugate gradient
algorithm that is used to minimize the LSDA energy functional
$E=E\left( Y_{\uparrow},Y_{\downarrow} \right)$.  What makes this
algorithm different from a standard 
Polak-Ribi\`{e}re conjugate gradient algorithm\cite{Shewchuk:94}
...
\cite{Car-Parrinello:85,Remler-Madden:90}.  
...
\noindent
To solve the Riccatti equation an iterative solution is setup by rewriting
Eq.~\ref{eq:b9} in the following form\cite{Car-Parrinello:88}.
\begin{equation}
\alpha^2 XCX^{\dag} + \alpha X(B-I) + \alpha (B^{\dag}-I)X^{\dag} 
 + \alpha(X + X^{\dag}) = I-A
\end{equation}
...
A description of the parameters $Q_e$, $E_{e0}$, $Q_R$, and $E_{R0}$ and their relationship
to the input parameters, $P_{electron}$, $T_{electron}$, $P_{ion}$, and $T_{ion}$, is given 
in section.  
