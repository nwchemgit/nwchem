%
% $Id: pspw.tex,v 1.39 2004-05-17 20:19:29 edo Exp $
%
\label{sec:pspw}
...
To perform an actual calculation the following is used (Section \ref{sec:task}).  
\begin{verbatim}
...
directory \\
\verb+ $NWCHEM_TOP/src/nwpw/libraryps/library2+
...
(nwchem-users@emsl.pnl.gov or eric.bylaska@pnl.gov)  
...
\normalsize
\section{PSPW Tutorial 3: Running a Simulated Annealing Car-Parrinello Simulation}
\label{sec:pspw_annealing}



\normalsize
\section{PSPW Tutorial 4: optimizing a unit cell and geometry for Silicon-Carbide}
...
\item PAW only works with units cells that have periodic boundary conditions.
\item PAW is not interfaced to driver, stepper, and vib modules.
\end{itemize}
