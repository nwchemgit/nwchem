...
module is reset to $1.0d-6$, a value smaller than the default of
$1.0d-4$.  The default is appropriate for determination of the
...
{\tt task} directivee invokes an MP2 optimization, instead of the scf
optimization.
...
documentation for describing the format of directives.  Understanding
this in detail should not be necessary since there are many examples.
...
character.  A semicolon (\verb+;+) may be also used to indicate the
end of an input line.  Thus, each physical line of input may contain
multiple logical lines of input.  For example,
\begin{verbatim}
  geometry
   O 0  0     0
   H 0  1.430 1.107
...
\end{verbatim}
which is one physical input line that corresponds to five logical
...
The comment character is \verb+#+ (the hash or pound symbol).  All
characters after \verb+#+ up to the physical end of line are ignored.
...
have multiple input lines, and must be terminated with an END
...
Using the directive \verb+VECTORS+ (Section \ref{sec:vectors}) as an
example
\begin{verbatim}
