...
labeled |verb+CONSTANTS+ will saty frozen to the value given in the
...
inferred by comparing inter-atomic distances with the sum of the van
der Waals radii of the two atoms involved in a possible bond, times a
scaling factor. The scaling factor is an input parameter of
\verb+ZCOORD+ which maybe changed from its default value of 1.2 . Under
some circumstances ( unusual bonding, bond dissociation, ...) it will
be necessary to augment the automatically generated list of internal
coordinates to force some specific internal coordinates to be included
in among the internal coordinates.  This is accomplished by including
the optional directive {\tt ZCOORD} within the geometry directive.  The
general form of the \verb+ZCOORD+ directive is as follows:
\begin{verbatim}
  ZCOORD
     [cvr_scaling < real value > ] 
...
\begin{itemize}
\item {\tt cvr_scaling} --- scaling factor applied to van der Waals radii.
...
\item {\tt ijklnb} --- a linear bend.  This defines two angles
  ...
  \verb+l+ must not be collinear with \verb+i-j-k+.  The two bends are 
  constructed to be within and perpendicular to the plane containing 
  the atoms.
\end{itemize}   
...
defined Z-matrix ( see \verb+ZMATRIX+ directive ) or by automatic
definition of internal coodinates by the program ( see 
\verb+AUTOZ+ directive ). Currently it is possible to freeze
internal coordinates during an energy minimization when they
have been defined by means of the \verb+ZMATRIX+ directive only.
( the \verb+AUTOZ+ option defines a redundant set of internal
coordinates, and in the context of redundant coordinates, the
code does not know how to freeze selected coordinates yet! ). In
the contex of the \verb+ZMATRIX+ directive then, internal coordinates
that have been entered by numerical value, or that have been
specified by a symbolic name labeled as \verb+CONSTANTS+ will
be frozen during an energy minimization.

When atoms have been specified by their cartesian coordinates, it
is possible to freeze the cartesian position of selected atoms.
This is useful for such purposes as optimizing a molecule
